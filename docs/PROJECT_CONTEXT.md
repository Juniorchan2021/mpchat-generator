...
- **外部文章检测与优化**：对已有博客文章进行 SEO/GEO 评分、AI 检测、多模式优化，并可直接发布
- **多平台发布**：生成内容一键分发到 Dev.to、Hashnode、Medium（API 直发 + 格式降级）、Paragraph（API 直发）、LinkedIn 等平台；外部文章页面同样支持四大平台发布
...
| Paragraph.xyz API | 文章直发（需 API Key） |
...
| 组件 | 文件 | 职责 |
|------|------|------|
| `WorkspaceClient` | `components/WorkspaceClient.tsx` | 软文工作台全流程：配置→生成→预览→SEO/GEO分析→导出→发布（含 Medium API 直发 + **Paragraph** 直发）→AI检测→**翻译** |
| `ExternalClient` | `components/ExternalClient.tsx` | 外部文章：粘贴→分析(检查清单)→5种优化模式→AI检测→**翻译**→**发布**（Dev.to/Hashnode/Medium/Paragraph） |
...
| POST | `/api/v1/publish/medium` | API 直发 / 格式预览 | 有 `medium_token` 时直发；无则返回 Markdown 预览 ← Phase 2 改造 |
| POST | `/api/v1/publish/paragraph` | API 直发 | 需要 Paragraph API Key ← Phase 2 新增 |
